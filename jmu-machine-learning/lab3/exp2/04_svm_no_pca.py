"""
实验三 - 04: SVM 预测录取率（归一化 + 不降维）
运行：在 lab3/ 目录下  python exp2/04_svm_no_pca.py
"""
import os
import time
import pandas as pd
...
}).to_csv('results/04_no_pca_predictions.csv', index=False, encoding='utf-8-sig')

pd.DataFrame({
    'MSE': [mse], 'RMSE': [rmse], 'R2': [r2],
    'train_time': [train_time], 'pred_time': [pred_time],
    'n_features': [X.shape[1]]
}).to_csv('results/04_no_pca_metrics.csv', index=False)

# ── 7. 绘图 ──────────────────────────────────────────────────
fig, axes = plt.subplots(1, 2, figsize=(13, 5))
fig.suptitle('SVM 预测结果（归一化 + 不降维，7个特征）', fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig('results/04_no_pca_plot.png', dpi=150, bbox_inches='tight')
plt.close()
print("\n✓ 图片  → results/04_no_pca_plot.png")
print("✓ 结果  → results/04_no_pca_predictions.csv")
print("✓ 指标  → results/04_no_pca_metrics.csv")
